...
-- Insert sample data
INSERT INTO Employees (id, name, salary) VALUES(1, 'Alice', 50000),(2, 'Bob', 60000),(3, 'Charlie', 55000);
...
ALTER TABLE Employees RENAME TO Staff3;

-- Drop the department column
ALTER TABLE Staff3 DROP COLUMN department;
desc staff3;

-- Add an auto-incrementing id (MySQL only — usually needs to be created at table definition)
ALTER TABLE Staff3 MODIFY id INT AUTO_INCREMENT;

-- Reorder the full_name column after id (MySQL only)
ALTER TABLE Staff3 MODIFY full_name VARCHAR(100) AFTER salary;
desc staff3;
select * from staff3;
-- Oracle-only: Make a column invisible in SELECT *
 ALTER TABLE Staff3 MODIFY salary decimal(12,2) INVISIBLE;
