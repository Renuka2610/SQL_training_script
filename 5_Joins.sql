...
INSERT INTO employeesJoin VALUES(105, 'Alice', 1),(106, 'Bob', 2),(103, 'Charlie', 3),(104, 'David', NULL),(105, 'Eve', 5); -- no matching department
-- -------------------------- DUMMY DATA --------------------------------------------------------------------
-- 1: INNER JOIN: Matches only where dept_id exists in both tables
SELECT emp_id, emp_name, dept_name, e.dept_id 
FROM employeesJoin e
 JOIN departmentsJoin d ON e.dept_id = d.dept_id ;
...
-- Select emp_id, emp_Name, dept_name from employeesJoin e  join departmentsJoin d where e.dept_id = d.dept_id;
 Select * from employeesJoin e  right join departmentsJoin d on e.dept_id = d.dept_id;
 update  employeesJoin set dept_id = 4 where emp_id = 104 or emp_id = 105;
